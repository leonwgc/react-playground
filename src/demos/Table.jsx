...
export default function App() {
  ...
  const [currentClickNegativeKeywordsList, setCurrentClickNegativeKeywordsList] = useState([]);

  const ref = useRef(); // hold data list

  useMount(() => {
    get('/dplatform-cloud-gateway/click-ppc/preset-negative-keywords?languageCode=en').then(res => {
  ...
  useUpdateEffect(() => {
    ...
    const selectedKeywords = formatedClickNegativeKeywordData?.map(item => {
      ...
      const parentSelected = item.children.length === selectedChildren.length;
      if (parentSelected) {
        return [
          {
